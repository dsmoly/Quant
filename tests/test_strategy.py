...
def test_gap_channel_is_off_by_default():
    """The regime switch is disabled unless explicitly enabled.

    The sweep in experiments/regime_sweep.py does not support it out of sample,
    so phi must not move with the competitive gap in the default configuration.
    """
    pol = AmbiguityPolicy(nash_depth=0.02)
    assert pol.gap_sensitivity == 0.0
    _, wide = pol(mu=0.02 * 1.30, sigma=0.01, tail=0.0)
    _, at_nash = pol(mu=0.02, sigma=0.01, tail=0.0)
    assert wide == at_nash == pytest.approx(pol.phi_base)


def test_gap_channel_is_aggressive_when_enabled():
    pol = AmbiguityPolicy(nash_depth=0.02, gap_sensitivity=1.0)
    ...
    assert phi_at_nash == pytest.approx(pol.phi_base)


def test_policy_deadband_suppresses_small_gaps():
    pol = AmbiguityPolicy(nash_depth=0.02, gap_deadband=0.05, gap_sensitivity=1.0)
    _, phi = pol(mu=0.02 * 1.04, sigma=0.01, tail=0.0)
    assert phi == pytest.approx(pol.phi_base)
...
def test_policy_is_defensive_when_flow_is_toxic():
    """The sweep's actual finding: phi_alpha should track markout, not competition."""
    pol = AmbiguityPolicy(nash_depth=0.02)
    benign, _ = pol(mu=0.02, sigma=0.01, tail=0.0, toxicity=0.0)
    mild, _ = pol(mu=0.02, sigma=0.01, tail=0.0, toxicity=0.004)
    toxic, _ = pol(mu=0.02, sigma=0.01, tail=0.0, toxicity=0.012)
    assert benign == pytest.approx(pol.phi_alpha_base)
    assert mild == pytest.approx(benign)         # inside the toxicity deadband
    assert toxic > benign


def test_policy_ignores_favourable_markout():
    """Negative markout means flow is benign; it must not make us aggressive."""
    pol = AmbiguityPolicy(nash_depth=0.02)
    flat, _ = pol(mu=0.02, sigma=0.01, tail=0.0, toxicity=0.0)
    good, _ = pol(mu=0.02, sigma=0.01, tail=0.0, toxicity=-0.02)
    assert good == pytest.approx(flat)


def test_markout_estimator_signs_and_settles():
    """Selling into a rising market and buying into a falling one both score adverse."""
    est = _estimator()
    est.observe_fill_for_markout(0.0, 100.0, "ask")     # we sold at 100
    est.settle_markouts(1.0, 101.0)                     # not old enough yet
    assert est.toxicity == 0.0
    est.settle_markouts(10.0, 101.0)                    # price rose after we sold
    assert est.toxicity > 0

    est = _estimator()
    est.observe_fill_for_markout(0.0, 100.0, "bid")     # we bought at 100
    est.settle_markouts(10.0, 99.0)                     # price fell after we bought
    assert est.toxicity > 0

    est = _estimator()
    est.observe_fill_for_markout(0.0, 100.0, "ask")
    est.settle_markouts(10.0, 99.0)                     # price fell after we sold: benign
    assert est.toxicity < 0


def test_markout_estimator_converges_to_the_true_impact():
    est = _estimator()
    for i in range(2_000):
        est.observe_fill_for_markout(float(i), 100.0, "ask")
        est.settle_markouts(float(i) + 10.0, 100.003)   # a steady 0.003 adverse move
    assert est.toxicity == pytest.approx(0.003, rel=0.05)
...
def test_adaptive_strategy_logs_and_moves_its_ambiguity_vector():
    ...
    for i in range(400):
        ...
        mm.observe(ev)
    assert mm.phi_log[-1] == pytest.approx(tight_phi)   # gap channel off by default
    assert len(mm.phi_log) == len(mm.phi_alpha_log)

    # With the gap channel enabled the same evidence does move phi.
    mm2 = AdaptiveRAMM(policy=AmbiguityPolicy(nash_depth=0.02, gap_sensitivity=1.0),
                       mu0=0.02, resolve_every=5, adapt_reference=False)
    for i in range(20):
        ev = _event(filled=False, t=i * 0.25)
        ev.cover = 0.02
        mm2.observe(ev)
    before = mm2.phi_log[-1]
    for i in range(400):
        ev = _event(filled=False, t=100 + i * 0.25)
        ev.cover = 0.02 * 1.4
        mm2.observe(ev)
    assert mm2.phi_log[-1] > before


def test_adaptive_strategy_widens_when_it_sees_toxic_markout():
    mm = AdaptiveRAMM(policy=AmbiguityPolicy(nash_depth=0.02), mu0=0.02,
                      resolve_every=5, adapt_reference=False)
    for i in range(50):
        mm.observe(_event(filled=False, t=i * 0.5))
    benign = mm.phi_alpha_log[-1]
    mid = 100.0
    for i in range(600):
        t = 100 + i * 0.5
        mid += 0.004                                  # every fill is followed by a run-up
        ev = _event(filled=True, t=t, mid=mid)
        ev.side = "ask"                               # we keep selling into it
        mm.observe(ev)
    assert mm.phi_alpha_log[-1] > benign
